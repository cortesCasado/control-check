...
public interface AdministratorDashboardRepository extends AbstractRepository {
	...
	@Query("SELECT 100.00 * COUNT(i) / (SELECT COUNT(j) FROM Receipt j) FROM Receipt i WHERE i.paidEx = true")
	Double flaggedAs();

	@Query("SELECT 100.00 * COUNT(i) / (SELECT COUNT(j) FROM Receipt j) FROM Receipt i WHERE i.totalPriceEx.amount >= 500 AND i.totalPriceEx.currency = 'EUR'")
	Double ratioOfShouts();

	@Query("SELECT AVG(i.totalPriceEx.amount) FROM Receipt i WHERE i.totalPriceEx.currency = 'EUR'")
	Double avgMoneyCurrency1();

	@Query("SELECT STDDEV(i.totalPriceEx.amount) FROM Receipt i WHERE i.totalPriceEx.currency = 'EUR'")
	Double dvtMoneyCurrency1();

	@Query("SELECT AVG(i.totalPriceEx.amount) FROM Receipt i WHERE i.totalPriceEx.currency = 'USD'")
	Double avgMoneyCurrency2();

	@Query("SELECT STDDEV(i.totalPriceEx.amount) FROM Receipt i WHERE i.totalPriceEx.currency = 'USD'")
